"""Run HybridEngine evaluation over transition-pair JSON files."""
...
PAIR_RESULTS_FILENAME = "hybrid_pair_results.csv"
SUMMARY_FILENAME = "hybrid_summary.json"
CSV_COLUMNS = [
    "case_id",
    "source_file",
    "pair_id",
...
class FakeLLMClient:
    ...
    def complete(self, prompt: str) -> str:
        ...
        response = json.dumps(
            {"adjustments": {vital: None for vital in CANONICAL_VITAL_KEYS}},
            ensure_ascii=True,
            sort_keys=True,
        )
...
def _record_from_result(
    *,
    ...
    return {
        ...
        "rule_prediction_vitals": _metadata_vitals(metadata, "rule_prediction"),
        "llm_adjustments": metadata.get("llm_adjustments", {}),
...
def _metadata_vitals(metadata: dict[str, Any], key: str) -> dict[str, Any]:
    value = metadata.get(key, {})
    if not isinstance(value, dict):
        return {}
    vitals = value.get("vitals", {})
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate HybridEngine over transition-pair JSON files."
    )
    parser.add_argument(
        "input_path",
        type=Path,
    ...
    parser.add_argument(
        "output_dir",
        type=Path,
        help="Directory for hybrid_pair_results.csv and hybrid_summary.json.",
    )
    parser.add_argument(
        "-r",
        "--recursive",
